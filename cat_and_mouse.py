...
class Cat_and_Mouse:
    ...
    def __init__(self,rows=5, columns=5, mouseInitLoc=None, cheeseLocs=None,
        ...
        if mouseInitLoc is None: self.mouseInitLoc=[0,0]
        else: self.mouseInitLoc=list(mouseInitLoc)
        if cheeseLocs is None: self.cheeseLocs=[[rows-1,columns-1]]
        else: self.cheeseLocs=list(cheeseLocs)
        ...
        if self.offOfMap(self.mouseInitLoc):
            raise Exception("Error: mouse initial location out of bounds: {}".format(self.mouseInitLoc)) 
        for cheese in self.cheeseLocs:
            if cheese in self.catLocs:
                raise Exception("Error: Cat and cheese locations cannot overlap: {}".format(cheese))
        for cheese in self.cheeseLocs:      
            if self.offOfMap(cheese):
                raise Exception("Error: cheese location out of bounds: {}".format(cheese))
        for cat in self.catLocs:
            if self.offOfMap(cat):
                raise Exception("Error: cat location out of bounds: {}".format(cat))
        for sticky in self.stickyLocs:
                if sticky in self.slipperyLocs:
                    raise Exception("Error: Sticky and slippery locations cannot overlap: {}".format(sticky))
        for sticky in self.stickyLocs:
            if self.offOfMap(sticky):
                raise Exception("Error: sticky location out of bounds: {}".format(sticky))
        for slippery in self.slipperyLocs:
            if self.offOfMap(slippery):
                raise Exception("Error: slippery location out of bounds: {}".format(slippery))
        ...
        if self.rows == 1:
            ...
            self.numActions=8
            self.actions={0:'E',1:'W',2:'N',3:'S',4:'SE',5:'SW',6:'NE',7:'NW'}
            self.moves={0:[0,1],1:[0,-1],2:[-1,0],3:[1,0],4:[1,1],5:[1,-1],6:[-1,1],7:[-1,-1]}
    ...
    def offOfMap(self, coord):
        ...
        """
        if coord[0] >= self.rows or coord[0] < 0: return True
        if coord[1] >= self.columns or coord[1] < 0: return True
        return False
    ...
    def initState(self, newState=None):
        ...
        if newState == None: self.mouseLoc=list(self.mouseInitLoc)
        elif type(newState) == int:
            tmp=[newState//self.columns,newState%self.columns]
            if self.offOfMap(tmp):
                raise Exception("Error: initState: mouse location out of bounds: {}".format(tmp)) 
            self.mouseLoc=tmp
        else:
            if self.offOfMap(newState):
                raise Exception("Error: initState: mouse location out of bounds: {}".format(newState)) 
    ...
    def reset(self, initLoc=None, resetPrng=False):
        ...
        """
        if initLoc is None: self.mouseLoc=list(self.mouseInitLoc)
        else: self.initState(initLoc)
        if resetPrng: self.prng.seed(self.prngSeed) 
        self.renderedFiles=[]       
    ...
    def step(self, action):
        ...
        if (self.mouseLoc in self.stickyLocs) and self.prng.random() < self.stickProb:
            ...
            while (newMouseLoc in self.slipperyLocs) and self.prng.random() < self.slipProb:
                #slipped forward
                slipped=True
                slipStart=list(newMouseLoc) #need to remember this in case we slip off edge of map
        ...
        if self.offOfMap(newMouseLoc):
            if slipped: self.mouseLoc=slipStart
    ...
    def policy2gif(self, policy, mouseInitLoc, filebaseName, maxEpisodeLen=None, resetPrng=False):
        ...
        while not done:
            ...
            if (maxEpisodeLen is not None) and (i >= maxEpisodeLen): done=True
